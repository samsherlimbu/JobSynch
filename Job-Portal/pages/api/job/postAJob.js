...
const postAJob =  async (req, res) => {
    await ConnectDB();
    const data = req.body;
    const { user ,title,description , salary , company, location, company_logo, email , job_category , job_type , job_experience , job_vacancy , job_deadline } = data;
    const { error } = schema.validate({ user ,title,description , salary , company, location, company_logo, email , job_category , job_type , job_experience , job_vacancy , job_deadline });

    if (error) return res.status(401).json({ success: false, message: error.details[0].message.replace(/['"]+/g, '') });

    try {
        const creatingUser =  await Job.create({user , title,description , salary , company, location, company_logo, email , job_category , job_type , job_experience , job_vacancy , job_deadline });
