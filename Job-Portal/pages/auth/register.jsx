...
export default function Register() {
  ...
  const [error, setError] = useState({  email: "", password: "", name: '', role: "" });


  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.role) {
      setError({ ...error, role: "Choose your role" })
      return;
    }
    if (!formData.email) {
    ...
    if (data.success) {
      ...
      console.log ( "I am from the else block ");
    ...
    <>
      ...
      <div className='w-full h-screen bg-indigo-600 '>
        <div className="flex flex-col text-center items-center justify-center px-6 py-8 mx-auto h-screen lg:py-0 shadow-xl ">
          ...
          <div className="w-full rounded-lg shadow dark:border text-black md:mt-0 sm:max-w-md xl:p-0 bg-gray-200">
