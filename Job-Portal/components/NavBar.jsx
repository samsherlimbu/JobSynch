...
export default function NavBar() {
    ...
    return (
        <>
            <div className={`w-full ${scrolled ? "bg-[#6A64F1]" : "bg-blue-600"} px-6 h-14 bg-indigo-600 text-white flex items-center justify-between fixed top-0 left-0 z-50`}>
                <div className='px-2 h-full flex items-center justify-center'>
                    <p className='uppercase font-semibold tracking-widest text-lg cursor-pointer'> <Link href = {'/'}> <Image src='/logo.png' width={250} height={250}  alt="Picture of the author"/></Link>
                    </p> 
                </div>
                <div className='px-2 h-full hidden items-center justify-center lg:flex'>
                    <Link href={'/'} className="px-3 mx-4 text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500 ml-72 transition-all duration-700 " >Home</Link>
                    { (user && user?.role === 'recruiter') &&
                    <> 
                    <Link href={'/frontend/postAJob'} className="  text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500  transition-all duration-700 mx-4 p-3 " >Post Jobs</Link>
                    
                
                <Link href={'/frontend/postedJob'} className="  text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500  transition-all duration-700 mx-4 p-3  " >Posted Jobs</Link> 
                     
                    </>
                    }
                    {/* <Link href={'/frontend/displayJobs'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-1 uppercase" >View Jobs</Link> */}
                    { (user && user?.role === 'job seeker') &&
                    <Link href={'/frontend/dashboard'} className="  text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500 mx-4 p-3 transition-all duration-700 " >Dashboard</Link>
                    }
                    <Link href={'/frontend/contact'} className="  text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500 mx-4 p-3 transition-all duration-700 " >Contact</Link>

                    <Link href={'/frontend/services'} className="  text-base font-medium  uppercase hover:underline decoration-white hover:text-yellow-500 mx-4 p-3 transition-all duration-700 " >Services</Link>
                </div>
                <div className='px-2 h-full hidden items-center justify-center lg:flex ' >
                    {
                        user !== null ? (
                            <>
                                ...
                                <p className='text-lg px-4 font-semibold'>{user.name}
